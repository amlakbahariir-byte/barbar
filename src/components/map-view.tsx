  ...
  useEffect(() => {
    if (window.neshan && mapRef.current && !neshanMapRef.current) {
      try {
        ...
        map.on('move', () => {
            // You can get center coordinates on move
  ...
  useEffect(() => {
     if(!process.env.NEXT_PUBLIC_NESHAN_API_KEY) {
        toast({
            ...
            description: "لطفا کلید API نشان را در فایل .env قرار دهید."
        });
     }
  }, [toast]);
  ...
  return (
    <Card className="overflow-hidden">
      <CardContent className="p-0 relative h-[60vh] md:h-[70vh]">
        ...
        {!isMapReady && !process.env.NEXT_PUBLIC_NESHAN_API_KEY && (
            <div className="absolute inset-0 flex items-center justify-center bg-background/80 backdrop-blur-sm z-[1001]">
                <div className="text-center p-4">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">برای استفاده از نقشه، لطفاً کلید API خود را از وب‌سایت نشان دریافت کرده و در فایل `.env` با نام `NEXT_PUBLIC_NESHAN_API_KEY` قرار دهید.</p>
                </div>
            </div>
        )}
